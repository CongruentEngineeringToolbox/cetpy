...
class DataSet(Block):
    ...
    def df_errors(self) -> pd.DataFrame:
        ...
        DataSet.df
        """
        df = self.raw_df[self.raw_df.errored]
        error_filter = self.error_filter
        if error_filter is not None and len(error_filter > 0):
            df = apply_filter(df, error_filter, join_and=False)
        return df
    ...
    def get_neural_network_regressor(
            self, output_key: str,
            input_keys: str | List[str] | None = None,
