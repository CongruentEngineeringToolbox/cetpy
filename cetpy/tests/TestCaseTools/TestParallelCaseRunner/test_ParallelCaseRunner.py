"""
ParallelParallelCaseRunner Test
=======================

This file implements tests for ParallelParallelCaseRunner.
"""
...
from cetpy.CaseTools import ParallelCaseRunner
...
class TestParallelCaseRunner(TestBlock):
    ...
    @pytest.fixture
    def test_class(self) -> type[ParallelCaseRunner]:
        return ParallelCaseRunner
        ...
        for par in parameters:
            if par in metafunc.fixturenames:
                metafunc.parametrize(
                    par,
                    np.round(
                        np.linspace(*getattr(self, par + "_range"), self.resolution), 3
                    ),
                )
    ...
    def test_catch_errors_true(self, test_class, init_kwargs):
        c = test_class(
            ...
            sub_method="ese",
            n_cases=2,
            output_properties=["errored_calculation"],
            **init_kwargs
        )
        output_df = c.output_df
        assert isinstance(output_df, pd.DataFrame)
